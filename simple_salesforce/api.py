...
+          URL feature i.e. ` https://test.my.salesforce.com
        ...
        * session -- Custom requests session, created in calling code. This
        ...
        if self.custom_url:
            self.base_url = ('{instance}/services/data/v{version}/'
                             .format(instance=self.custom_url,
                                     version=self.sf_version))
            self.apex_url = ('{instance}/services/apexrest/'
                             .format(instance=self.custom_url))
            self.bulk_url = ('{instance}/services/async/{version}/'
                             .format(instance=self.custom_url,
                                     version=self.sf_version))
            self.tool_url = ('{base}tooling/').format(base=self.base_url)
            self.soap_url = ('{instance}/services/SOAP/T/v{version}/'
                             .format(instance=self.custom_url,
                                     version=self.sf_version))
        else:
            ...
            self.bulk_url = ('https://{instance}/services/async/{version}/'
                             .format(instance=self.sf_instance,
                                     version=self.sf_version))
            self.tool_url = ('{base}tooling/').format(base=self.base_url)
            self.soap_url = ('https://{instance}/services/SOAP/T/v{version}/'
                             .format(instance=self.sf_instance,
                                     version=self.sf_version))
...
class SFType(object):
    ...
    def __init__(
        ...
        * session -- Custom requests session, created in calling code. This
        ...
        self.tool_url = (
            u'https://{instance}/services/data/v{sf_version}/tooling/sobjects'
            '/{object_name}/'.format(instance=sf_instance,
                                     object_name=object_name,
                                     sf_version=sf_version))
    ...
    def toolingUpdate(self, record_id, data, raw_response=False, headers=None):
        """Updates a Tooling API SObject using a PATCH to
        `.../{object_name}/{record_id}`.
        ...
        * headers -- a dict with additional request headers.
        ...
        result = self._call_salesforce(
            method='PATCH',
            url=urljoin(self.tool_url, record_id),
            data=json.dumps(data),
            headers=headers
        )
        return self._raw_response(result, raw_response)

    def toolingUpdateSOAP(self, record_id, object_status, object_type):
        """Updates a Tooling API SObject using a PATCH to
        `.../{object_name}/{record_id}`.
    ...
    def _call_update_soap(self, **kwargs):
        """Utility method for performing SOAP Update call to Salesforce.

        Returns a `requests.result` object.
        """
        # headers = {
            # 'AllOrNoneHeader': 'application/json',
            ...
            # 'SessionHeader': self.session_id
            # 'Content-Type': "application/soap+xml"
            # 'X-PrettyPrint': '1'
        # }

        request = u"""<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:urn="urn:tooling.soap.sforce.com" xmlns:urn1="urn:sobject.tooling.soap.sforce.com">

   <soapenv:Header>
      <urn:AllOrNoneHeader>
         <urn:allOrNone>{allOrNone}</urn:allOrNone>
      </urn:AllOrNoneHeader>
      <urn:SessionHeader>
   ...
   <soapenv:Body>
      <urn:update>
         ...
         <urn:sObjects xsi:type="{objectType}">
            <!--Zero or more repetitions:-->
            <urn:Status>{activeStatus}</urn:Status>
            <urn:Id>{Id}</urn:Id>
         </urn:sObjects>
      </urn:update>
   </soapenv:Body>
</soapenv:Envelope>""".format(allOrNone=True,
        ...
        additional_headers = kwargs.pop('headers', dict())
        headers.update(additional_headers or dict())
        # response = requests.post(url = self.soap_url, headers = headers, data = encoded_request, verify = False)
        response = requests.post(url = self.soap_url, data = encoded_request, verify = False)
